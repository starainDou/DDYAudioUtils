...
class DDYCafViewController: UIViewController, AVAudioPlayerDelegate {
    
    private lazy var recorder: DDYAudioRecorder = DDYAudioRecorder().ddy_then {
        $0.fileWriterDelegate = cafWriter
    }
    ...
    @objc private func playAction(_ sender: UIButton) {
        if changePlayState() {
            if FileManager.default.fileExists(atPath: ViewController.cafPath) {
                try? AVAudioSession.sharedInstance().setCategory(.playback)
                player = try? AVAudioPlayer(contentsOf: URL(fileURLWithPath: ViewController.cafPath))
