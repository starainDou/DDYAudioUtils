...
class DDYAmrViewController: UIViewController {
    
    private lazy var recorder: DDYAudioRecorder = DDYAudioRecorder().ddy_then {
        $0.fileWriterDelegate = amrWriter
    }
    ...
    @objc private func playAction(_ sender: UIButton) {
        if changePlayState() {
            if FileManager.default.fileExists(atPath: ViewController.amrPath) {
                try? AVAudioSession.sharedInstance().setCategory(.playback)
                amrPlayer = DDYAmrPlayer(fileName: ViewController.amrPath)
                amrPlayer?.play()
                amrPlayer?.onAudioPlayerDidFinishPlaying = { [weak self] (isFinished) in
                    DispatchQueue.main.async {
